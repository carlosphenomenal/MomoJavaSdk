...
import io.github.carlosphenomenal.Dtos.TransferBody;
...
public class MomoDisbursement extends MomoProduct {
     ...
     * @param httpClient    the HttpClient to use
     * @param objectMapper  the ObjectMapper for JSON serialization
     * @param transferBody  the transfer details
     * @param referenceId   the unique reference ID for this transfer request
     * @return the reference ID of the transfer
     */
    public String transfer(MomoMerchant momoMerchant, HttpClient httpClient, ObjectMapper objectMapper, TransferBody transferBody, String referenceId) {
        try {
            String accessToken = momoMerchant.getAccessToken(this);
            String url = momoMerchant.getTargetEnvironment().getBaseUrl() + "/disbursement/v1_0/transfer";
            String jsonPayload = objectMapper.writeValueAsString(transferBody);
            ...
            if (response.statusCode() == 202) {
                log.info("[MoMo][TRANSFER] Initiated successfully. TransferRef={}", referenceId);
                return referenceId;
            } else {
                throw new RuntimeException("Transfer failed with HTTP status: " + response.statusCode());
            }
        } catch (Exception e) {
     ...
     * @param referenceId   the reference ID of the transfer to check
     * @return the {@link TransferStatus} of the transfer
     */
    public TransferStatus checkTransferStatus(MomoMerchant momoMerchant, HttpClient httpClient, ObjectMapper objectMapper, String referenceId) {
        try {
            String accessToken = momoMerchant.getAccessToken(this);
            String url = momoMerchant.getTargetEnvironment().getBaseUrl() + "/disbursement/v1_0/transfer/" + referenceId;
            ...
            if (response.statusCode() == 200) {
                return objectMapper.readValue(response.body(), TransferStatus.class);
            } else {
                throw new RuntimeException("Check transfer status failed with HTTP status: " + response.statusCode() + ", Body: " + response.body());
            }
        } catch (Exception e) {
            throw new RuntimeException("Failed to check transfer status", e);
        }
    }
